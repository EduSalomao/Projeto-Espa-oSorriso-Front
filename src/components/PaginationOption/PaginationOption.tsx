...
const PaginationOption = ({page, total, limit, onPageChange}) => {
  const totalPages = Math.ceil(total / limit);
  const isPrevDisabled = page <= 1;
  const isNextDisabled = page >= totalPages;
  const handleNextPage = () => {
    if (page < totalPages) {
      onPageChange(Number(page) + 1);  // Chama a função de mudança de página para avançar
    }
  };
  ...
  const handlePrevPage = () => {
    if (page > 1) {
      onPageChange(page - 1);  // Chama a função de mudança de página para voltar
    }
  };
  ...
  return (
    ...
    >
      ...
      <img
        ...
        style={buttonStyle(isPrevDisabled)}
      />
